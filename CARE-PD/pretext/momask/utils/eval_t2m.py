...
def evaluation_vqvae(out_dir, val_loader, net, writer, ep, best_fid, best_div, best_top1,
                     best_top2, best_top3, best_matching, eval_wrapper, best_mpjpe, save=True, draw=True):
    net.eval()
    ...
    mpjpe = mpjpe / num_poses * 1000
    pampjpe = pampjpe / num_poses * 1000
    acc_err = acc_err / num_acc * 1000

    fid = calculate_frechet_distance(gt_mu, gt_cov, mu, cov)

    msg = "--> \t Eva. Ep %d:, FID. %.4f, MPJPE. %.4f, PAMPJPE. %.4f, Acc Error. %.4f"%\
          (ep, fid, mpjpe, pampjpe, acc_err)
    # logger.info(msg)
    print(msg)
    ...
    net.train()
    return best_fid, -1, -1, -1, -1, -1, best_mpjpe, mpjpe, writer
    # return best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer

@torch.no_grad()
def evaluation_vqvae_inference(val_loader, net, eval_wrapper):
    net.eval()
    ...
    net.train()
    return fid, mpjpe, pampjpe, acc_err
